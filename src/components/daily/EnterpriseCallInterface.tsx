  /**
 * ✨ ENHANCED: Enterprise-Grade Medical Video Call Interface
 *
 * This version adds:
 * 1. ✅ Comprehensive SDK error handling (all Daily.co events)
 * 2. ✅ Call health monitoring with heartbeat checks
 * 3. ✅ Explicit track cleanup to prevent memory leaks
 * 4. ✅ Stable event handler references (fixes stale closures)
 * 5. ✅ CallKit and Foreground Service integration
 * 6. ✅ Optimized track sync (reduces unnecessary retries)
 *
...
// ✨ NEW: Import native platform managers
...
interface DailyParticipant {
  session_id: string; // ✨ NEW: Unique session identifier
  user_name?: string;
  local?: boolean;
...
// ✅ FIX #1: Track state validation helper
const isTrackLive = (track: DailyMediaStreamTrack | null): boolean => {
  ...
  if (__DEV__ && track && !isLive) {
    devLog(`🎥 Track state check: readyState=${readyState}, id=${(track as any)?.id}`);
  }
...
const EnterpriseCallInterface: React.FC<EnterpriseCallInterfaceProps> = ({
  ...
  const [callState, setCallState] = useState({
    ...
    isCameraMuted: false, // ✅ LOCAL camera state only
    isMicMuted: false, // ✅ LOCAL mic state only
    callDuration: 0,
    callStartTime: null as Date | null,
    // ✅ NEW: Track remote states separately
    isRemoteCameraOff: false,
    isRemoteMicOff: false, // ✅ NEW: Track remote mic state
  });
  ...
  // ✅ FIX: Memory pressure warning state
  const [showMemoryWarning, setShowMemoryWarning] = useState(false);

  // ✨ NEW: Track current remote participant to prevent mixing
  ...
  // ✅ FIX: Sync camera preference with background manager (simplified)
  useEffect(() => {
  ...
  // ✨ NEW: CallKit/Foreground service tracking
  ...
  // ✨ NEW: Remote track fallback timeout (when tracked remote loses video)
  ...
  // ✨ NEW: Scan for any remote participant with a live video track and switch
  const scanForRemoteWithVideo = useCallback(() => {
    ...
    try {
      ...
      if (foundSessionId && foundTrack) {
        devLog('🎥 Switching tracked remote to', foundSessionId, 'track', (foundTrack as any)?.id);
  ...
  // ✨ FIX #5: Track lifecycle handlers with strict local/remote separation
  const onTrackStarted = useCallback(
    (ev?: unknown) => {
      ...
      // ✅ Handle video tracks
      if (kind === 'video') {
        // ✅ CRITICAL: Strict local check - ONLY update LOCAL state
        if (participant.local === true) {
          ...
          if (localTrack && (localTrack as any)?.readyState === 'live') {
            devLog('📹 [LOCAL] Video track started id=', (localTrack as any)?.id);
            setCallState(prev => ({
              ...prev,
              localVideoTrack: localTrack,
              // ⚠️ NEVER update isCameraMuted here - only periodic sync does this
            }));
          }
          return; // ✅ Early return - remote events cannot affect local state
        }

        // ✅ Handle remote participants (only if local === false)
        if (participant.local === false) {
          if (participant.session_id === currentRemoteSessionId) {
            ...
            if (remoteTrack) {
              devLog('📹 [REMOTE] Video track started for', participant.session_id, 'id=', (remoteTrack as any)?.id);
              setCallState(prev => ({
                ...prev,
                videoTrack: remoteTrack,
                isRemoteCameraOff: false, // ✅ Remote camera is ON
              }));
              setConnectionState(prev => ({ ...prev, hasRemoteVideo: true }));
            }
            ...
            devLog('🎥 [REMOTE] Tracking remote from track-started:', participant.session_id, 'id=', (remoteTrack as any)?.id);
          ...
          // ✅ FIX: Log unexpected participant.local value
          devWarn('⚠️ video track-started: Participant with undefined/null local property:', participant);
        }
      }

      // ✅ NEW: Handle audio tracks
      if (kind === 'audio') {
        // ✅ CRITICAL: Strict local check - ONLY update LOCAL state
        if (participant.local === true) {
          devLog('🎤 [LOCAL] Audio track started');
        ...
        if (participant.local === false) {
          if (participant.session_id === currentRemoteSessionId) {
            devLog('🎤 [REMOTE] Audio track started for', participant.session_id);
            setCallState(prev => ({
              ...prev,
              isRemoteMicOff: false, // ✅ Remote mic is ON
            }));
          }
        } else {
          devWarn('⚠️ audio track-started: Participant with undefined/null local property:', participant);
        }
      }
    },
  ...
  const onTrackStopped = useCallback(
    (ev?: unknown) => {
      ...
      if (kind === 'video') {
        // ✅ CRITICAL: Strict local check - ONLY update LOCAL state
        if (participant.local === true) {
          devLog('📹 [LOCAL] Video track stopped');
          setCallState(prev => ({
            ...prev,
        ...
        if (participant.local === false) {
          if (participant.session_id === currentRemoteSessionId) {
            devLog('📹 [REMOTE] Video stopped for tracked participant', participant.session_id);
            setCallState(prev => ({
              ...prev,
              videoTrack: null,
              isRemoteCameraOff: true, // ✅ Remote camera is OFF
            }));
          ...
          // ✅ FIX: Log unexpected participant.local value
          devWarn('⚠️ track-stopped: Participant with undefined/null local property:', participant);
        }
      }
      ...
      if (kind === 'audio') {
        // ✅ CRITICAL: Strict local check - ONLY update LOCAL state
        if (participant.local === true) {
          devLog('🎤 [LOCAL] Audio track stopped');
        ...
        if (participant.local === false) {
          if (participant.session_id === currentRemoteSessionId) {
            devLog('🎤 [REMOTE] Audio stopped for tracked participant', participant.session_id);
            setCallState(prev => ({
              ...prev,
              isRemoteMicOff: true, // ✅ Remote mic is OFF
            }));
          }
        } else {
          devWarn('⚠️ audio track-stopped: Participant with undefined/null local property:', participant);
        }
      }
    },
  ...
  const clearAllIntervals = useCallback(() => {
    ...
    // ✨ NEW: Clear remote track fallback timeout
    if (remoteTrackFallbackTimeoutRef.current) {
      clearTimeout(remoteTrackFallbackTimeoutRef.current);
      remoteTrackFallbackTimeoutRef.current = null;
    }
  ...
  // ❌ REMOVED: syncAllParticipantTracks (replaced by event-driven updates)

  // ✅ FIX: Use refs for stable handler identity (prevents stale closures)
  const handleParticipantChange = useCallback(
    (ev?: unknown) => {
      ...
      // ✅ FIX #2: Strict boolean check for local participant
      if (participant.local === true) {
        try {
      ...
      // ✅ FIX #2: Explicit remote participant check
      if (participant.local === false) {
        try {
          ...
          if (!currentRemoteSessionId && participant.tracks?.video?.persistentTrack) {
            ...
            devLog('🎥 Tracking new remote:', participant.session_id);
            setCurrentRemoteSessionId(participant.session_id);
            setCallState(prev => ({
              ...prev,
      ...
      // ✅ FIX #2: Log unexpected participant.local value
      if (participant.local === undefined || participant.local === null) {
        devWarn('⚠️ participant-change: Participant with undefined/null local property:', participant);
        return;
      }
    },
    [callObject, currentRemoteSessionId]
  ...
  // ❌ REMOVED: handleParticipantChangeRef (using direct callbacks)

  // ✅ ENHANCED: Participant left with explicit track cleanup
  const handleParticipantLeft = useCallback(
    (ev?: unknown) => {
      ...
      if (participant.session_id === currentRemoteSessionId) {
        devLog('🧹 Clearing tracked remote:', participant.session_id);
  ...
  // ❌ REMOVED: handleParticipantLeftRef (using direct callbacks)

  // ✨ NEW: Comprehensive error handling for all Daily.co SDK events
  const handleMeetingError = useCallback((ev?: unknown) => {
    const event = ev as DailyParticipantEvent | undefined;
    devError('🎬 ❌ Meeting error:', event?.message || event?.errorMsg);
    setAppState('error');
    setConnectionState(prev => ({
      ...prev,
      isStuck: true,
  ...
  const handleNonFatalError = useCallback((ev?: unknown) => {
    const event = ev as DailyParticipantEvent | undefined;
    devWarn('⚠️ Non-fatal error:', event?.message || event?.errorMsg);
    // Don't change app state, just log for monitoring
  }, []);

  const handleCameraError = useCallback((ev?: unknown) => {
    const event = ev as DailyParticipantEvent | undefined;
    devError('📹 Camera error:', event?.message || event?.errorMsg);
  ...
  const handleNetworkConnectionEvent = useCallback((ev?: unknown) => {
    ...
    devLog(`🌐 Network connection event: ${eventType}`);
  ...
  // ✨ NEW: Call health monitoring with heartbeat
  useEffect(() => {
    ...
    devLog('💓 Starting call health monitoring...');
    ...
    heartbeatIntervalRef.current = setInterval(async () => {
      try {
        ...
        if (counts.present < 1) {
          devWarn('💔 Heartbeat check: No participants present - call may be dead');
          setConnectionState(prev => ({
            ...prev,
            isStuck: true,
            stuckReason: 'connection_lost',
        ...
        devLog(`💓 Heartbeat: ${counts.present} participants, ${Math.round(timeSinceLastHeartbeat / 1000)}s since last check`);

      } catch (error) {
        devError('💔 Heartbeat check failed - call object may be dead:', error);
        setConnectionState(prev => ({
          ...prev,
          isStuck: true,
          stuckReason: 'connection_lost',
  ...
  const backgroundManagerListener: VideoCallBackgroundListener = useMemo(
    () => ({
      ...
      onAppStateChange: (appState, bgCallState) => {
        ...
        // ✅ FIX #2: Sync current camera state to background manager BEFORE transition
        if (appState === 'background' || appState === 'inactive') {
          const currentCameraEnabled = callObject ? callObject.localVideo() : !callState.isCameraMuted;
          devLog(`🎥 Syncing camera preference to background manager: ${currentCameraEnabled}`);
          VideoCallBackgroundManager.getInstance().setUserCameraPreference(currentCameraEnabled);
        
          // 🔧 NEW: Clear camera toggle lock when backgrounding
          if (cameraToggleInFlightRef.current) {
            devLog('🎥 🔧 Clearing camera toggle lock due to background transition');
        ...
        if (enabled && callObject) {
          try {
            ...
            if (freshTrack && (freshTrack as any)?.readyState === 'live') {
              devLog(`🎥 🔧 Fetching fresh track after resume: id=${(freshTrack as any)?.id}`);
              setCallState(prev => ({
                ...prev,
                isCameraMuted: false,
              ...
              devWarn(`🎥 Camera enabled but no live track found yet`);
              setCallState(prev => ({
                ...prev,
                isCameraMuted: false,
                localVideoTrack: null,
      ...
      onQualityDegraded: (fromQuality, toQuality) => {
        ...
        // ✅ FIX: Show warning when memory pressure detected but camera kept on
        if (toQuality === 'memory_pressure_warning') {
          setShowMemoryWarning(true);
          // Auto-hide after 5 seconds
          setTimeout(() => setShowMemoryWarning(false), 5000);
        }
      },
    }),
    [onError]
...
// 🔧 FIX SOLUTION #2: Update event handler refs whenever handlers change (no effect re-run)
useEffect(() => {
...
// 🔧 FIX SOLUTION #2: Wrapper functions that always call current handler ref
...
const stableHandlers = useMemo(() => ({
  ...
  useEffect(() => {
    ...
    // ✅ FIX: Direct event handlers (no refs needed with proper dependencies)
    ...
    const onJoined = () => {
      ...
      // ✨ NEW: Start native platform services
      if (Platform.OS === 'ios') {
      ...
      // ✅ FIX: Events will handle track updates automatically (no polling needed)
    ...
    const onLeft = () => {
      ...
      // ✨ NEW: Stop native platform services
      if (Platform.OS === 'ios') {
    ...
    devLog('📡 Attaching comprehensive event listeners to call object');
    callObject
    ...
    if (autoJoin && appState === 'idle') {
      ...
      // ✨ NEW: Start native platform services for outgoing call
      if (Platform.OS === 'ios') {
  ...
  // ✨ NEW: Guard against accidental local/remote track id collision
  useEffect(() => {
    ...
    if (localId && remoteId && localId === remoteId) {
      devWarn('⚠️ Duplicate video track id detected for local and remote. Clearing local preview to resync.', localId);
      setCallState(prev => ({ ...prev, localVideoTrack: null }));
    }
  }, [callState.localVideoTrack, callState.videoTrack]);

  // ✅ FIX #3: Track state listeners for local video
  useEffect(() => {
    ...
    const handleTrackEnded = () => {
      devLog('📹 Local track ended event - clearing from state');
      setCallState(prev => ({
        ...prev,
        localVideoTrack: null,
        // ❌ REMOVED: isCameraMuted update - periodic sync handles this
      }));
    };
    ...
    if (track.readyState === 'ended') {
      devLog('📹 Local track already ended on mount');
      handleTrackEnded();
    }
  ...
  // ✅ FIX #3: Track state listeners for remote video
  useEffect(() => {
    ...
    const handleTrackEnded = () => {
      devLog('📹 Remote track ended event - clearing from state');
      setCallState(prev => ({
        ...prev,
        videoTrack: null,
      }));
    ...
    // Check current state immediately
    if (track.readyState === 'ended') {
      devLog('📹 Remote track already ended on mount');
      handleTrackEnded();
    }
  ...
  // ✅ FIX #6: Improved periodic camera state verification (LOCAL ONLY - reduced to 1s)
  useEffect(() => {
    if (appState !== 'joined' || !callObject || callType !== 'video') return;

    devLog('🔄 Starting periodic LOCAL camera state verification...');
      ...
      try {
        ...
        if (sdkLocalCameraState) {
          try {
            ...
            devWarn('🔄 Failed to fetch local track during sync:', err);
          }
        }
        ...
        setCallState(prev => {
          ...
          if (!flagInSync) {
            devWarn(`⚠️ 🔧 Resyncing LOCAL camera FLAG: ${prev.isCameraMuted} → ${!sdkLocalCameraState}`);
          }
          if (!trackInSync && sdkLocalCameraState && freshLocalTrack) {
            devWarn(`⚠️ 🔧 Resyncing LOCAL camera TRACK: fetched id=${(freshLocalTrack as any)?.id}`);
          }
        ...
        devError('🔄 🔧 HOLISTIC camera state sync error:', error);
      }
    }, 1000);

    return () => {
      devLog('🔄 Stopping periodic LOCAL camera state verification');
      clearInterval(syncInterval);
    };
  }, [appState, callObject, callType]);

  // ✅ NEW: Periodic mic state verification (LOCAL ONLY - 1s interval)
  useEffect(() => {
    if (appState !== 'joined' || !callObject) return;

    devLog('🔄 Starting periodic LOCAL mic state verification...');

    const syncInterval = setInterval(() => {
      try {
        // ✅ CRITICAL: Only sync LOCAL mic state from SDK
        const sdkLocalMicState = callObject.localAudio();

        // ⚠️ This ONLY updates isMicMuted (LOCAL mic state)
        ...
        setCallState(prev => {
          ...
          devWarn(`⚠️ Resyncing LOCAL mic state to SDK: ${sdkLocalMicState}`);
          return {
            ...prev,
            isMicMuted: !sdkLocalMicState, // ✅ LOCAL mic state only
            // ⚠️ isRemoteMicOff is NOT touched here - remote events handle it
          };
        });
      } catch (error) {
        devError('🔄 LOCAL mic state sync error:', error);
      }
    }, 1000); // ✅ 1 second for faster sync

    return () => {
      devLog('🔄 Stopping periodic LOCAL mic state verification');
      clearInterval(syncInterval);
    };
  }, [appState, callObject]);

  // ✅ FIX #2: Simplified camera toggle - LOCAL camera only
  const toggleCamera = useCallback(async () => {
    if (!callObject || appState !== 'joined' || cameraToggleInFlightRef.current) {
      return;
    }

    devLog('📹 [LOCAL] Toggling LOCAL camera...');
    cameraToggleInFlightRef.current = true;

    try {
      // ✅ CRITICAL: This ONLY affects LOCAL camera (callObject.localVideo)
      ...
      devLog(`📹 [LOCAL] Camera toggle: ${isCurrentlyOn} → ${targetState}`);

      // ✅ Sync LOCAL camera preference BEFORE SDK call
      VideoCallBackgroundManager.getInstance().setUserCameraPreference(targetState);

      // ✅ FIX #2: Call SDK and let track events handle state updates
      // This ONLY toggles LOCAL camera - remote participants are unaffected
      callObject.setLocalVideo(targetState);

      // ✅ Animation feedback for user responsiveness
      Animated.sequence([
      ...
      devError('📹 [LOCAL] Camera toggle failed:', error);
  ...
  // ✅ Mic toggle - LOCAL mic only, NO optimistic updates (trust SDK)
  const toggleMic = useCallback(() => {
    if (!callObject || appState !== 'joined') return;

    try {
      // ✅ CRITICAL: This ONLY affects LOCAL mic (callObject.localAudio)
      // Remote mic state is managed separately via participant events
      const isCurrentlyOn = callObject.localAudio();
      const targetState = !isCurrentlyOn;

      devLog(`🎤 [LOCAL] Toggling LOCAL mic: ${isCurrentlyOn} → ${targetState}`);

      // ✅ Call SDK - let periodic sync handle state updates
      // No optimistic updates - trust SDK as single source of truth
      callObject.setLocalAudio(targetState);

      // ✨ Update CallKit mute state (iOS only)
      if (Platform.OS === 'ios') {
      ...
      devError('🎤 [LOCAL] Mic toggle failed:', error);
  ...
  const handleEndCall = useCallback(async () => {
    ...
    try {
      ...
      // ✅ CRITICAL FIX: Call onEndCall BEFORE resetting state
      // This ensures parent clears navigation state before we set appState to 'idle'
      onEndCall();
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={[styles.videoContainer, uiState.isFullscreen && styles.fullscreenVideo]}>
          {callState.remoteParticipantCount > 0 ? (
            /* ✅ FIX #1: Use isTrackLive() to prevent frozen remote frames */
            isTrackLive(callState.videoTrack) ? (
              <>
                ...
                />
                {/* ✅ NEW: Show indicator when remote turns camera off (while track exists) */}
                ...
                {/* ✅ NEW: Show indicator when remote mutes mic */}
              ...
              /* ✨ Connected but no video - show audio-only indicator */
              <View style={styles.audioOnlyArea}>
                ...
                {/* ✅ NEW: Show mic indicator in audio-only mode too */}
                {callState.isRemoteMicOff && (
          ...
          {/* ✅ FIX: Memory pressure warning */}
          {showMemoryWarning && (
            <Animated.View style={[styles.memoryWarning, { opacity: fadeAnim }]}>
              <BlurView intensity={60} style={styles.memoryWarningBlur}>
                <Ionicons name="warning-outline" size={20} color="#FF9800" />
                <Text style={styles.memoryWarningText}>
                  Low memory detected. Consider turning off camera to improve performance.
                </Text>
              </BlurView>
            </Animated.View>
          )}
          ...
          {callType === 'video' && (
            ...
            >
              {/* ✅ FIX #1: Use isTrackLive() to prevent frozen frames */}
                ...
                {callType === 'video' && (
                  <TouchableOpacity
                    style={[
                      ...
                      /* ✅ FIX #4: Use SDK state directly for icon styling */
                  ...
                  >
                    <Ionicons
                      /* ✅ FIX #4: Icon reflects UI state (synced via periodic check) */
                      name={callState.isCameraMuted ? 'videocam-off' : 'videocam'}
                      size={28}
                      color="#fff"
                    />
...
const styles = StyleSheet.create({
  ...
  // ✅ NEW: Remote camera off indicator badge
  ...
  // ✅ NEW: Remote mic off indicator badge
  remoteMicOffBadge: {
    position: 'absolute',
  ...
  // ✅ FIX: Memory pressure warning styles
  memoryWarning: {
    position: 'absolute',
    top: 140,
    left: 20,
